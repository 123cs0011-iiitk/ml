#!/usr/bin/env python3
"""
KNN Model Standalone Trainer

Standalone training script for K-Nearest Neighbors model.

Usage:
    python backend/training/advanced_models/knn/trainer.py
    python backend/training/advanced_models/knn/trainer.py --max-stocks 100
"""
...
from training.advanced_models.knn.config import KNNConfig
...
class KNNTrainer(CommonTrainerMixin):
    """Standalone trainer for KNN model."""
    ...
    def save_model(self, model: KNNModel) -> str:
...
def setup_logging():
    ...
    log_file = os.path.join(log_dir, f'knn_training_{timestamp}.log')
...
def main():
    """Main function to run KNN training."""
    parser = argparse.ArgumentParser(description='Train KNN model on full dataset')
